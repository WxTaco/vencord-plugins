...
export async function generateEmbedImage(embedData: EmbedData, darkMode: boolean = false): Promise<string> {
    return new Promise((resolve) => {
        ...
        const actualContentHeight = currentY + 40; // Add some bottom padding
        const padding = 80;
        const watermarkHeight = 40;
        const finalWidth = Math.max(canvas.width + padding * 2, 700);
        const finalHeight = actualContentHeight + padding * 2 + watermarkHeight;
        ...
        if (finalCtx) {
            ...
            finalCtx.fillRect(watermarkX - 10, watermarkY + 5, watermarkWidth + 20, 30);

            // Subtle border
            finalCtx.strokeStyle = 'rgba(190, 24, 93, 0.2)';
            finalCtx.lineWidth = 1;
            finalCtx.strokeRect(watermarkX - 10, watermarkY + 5, watermarkWidth + 20, 30);
            ...
            finalCtx.fillText('Generated by Embed Builder 🌸', watermarkX, watermarkY + 23);
        }
