...
export async function generateEmbedImage(embedData: EmbedData, darkMode: boolean = false): Promise<string> {
    return new Promise((resolve) => {
        ...
        // Calculate content height and create final canvas with proper centering
        const contentHeight = Math.max(currentY + 20, 200);
        const padding = 60;
        const watermarkHeight = 50;
        const finalWidth = Math.max(canvas.width + padding * 2, 600);
        const finalHeight = contentHeight + padding * 2 + watermarkHeight;
        ...
        if (finalCtx) {
            ...
            const embedCenterY = (finalHeight - watermarkHeight - contentHeight) / 2 + padding / 2;
            ...
            // Add professional watermark at bottom
            const watermarkY = finalHeight - watermarkHeight;

            // Watermark background with gradient
            const watermarkGradient = finalCtx.createLinearGradient(0, watermarkY, 0, finalHeight);
            watermarkGradient.addColorStop(0, 'rgba(190, 24, 93, 0.03)');
            watermarkGradient.addColorStop(1, 'rgba(190, 24, 93, 0.08)');
            finalCtx.fillStyle = watermarkGradient;
            finalCtx.fillRect(0, watermarkY, finalWidth, watermarkHeight);

            // Subtle top border for watermark
            finalCtx.strokeStyle = 'rgba(190, 24, 93, 0.15)';
            finalCtx.lineWidth = 1;
            finalCtx.beginPath();
            finalCtx.moveTo(0, watermarkY);
            finalCtx.lineTo(finalWidth, watermarkY);
            finalCtx.stroke();

            // Watermark text - centered and professional
            finalCtx.font = 'bold 14px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
            finalCtx.fillStyle = 'rgba(190, 24, 93, 0.7)';
            finalCtx.textAlign = 'center';
            finalCtx.fillText('Generated by Embed Builder 🌸', finalWidth / 2, watermarkY + 30);
